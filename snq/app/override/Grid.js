Ext.define("LoginAppDemo.override.Grid", {
        ...
        */
        ...
        if (Ext.isChrome || Ext.isGecko || Ext.isSafari) { // local download
            ...
            form = this.add({
				...
				url: 'http://localhost/Fiddle-17j/index.html?method=echo&mimetype=application/vnd.ms-excel&filename=' + escape(title + ".xls"),
                items: [{
                    xtype: 'hiddenfield',
                    name: 'data',
                ...
                if (cm[j].xtype != 'actioncolumn' && (cm[j].dataIndex != '') && (includeHidden || !cm[j].hidden)) {
                    ...
                    if (cellType[k] !== "None") {
                        ...
                        if (cellType[k] == 'DateTime') {
							...
							console.log('DateTime: ' + Ext.Date.format(v, 'Y-m-d'));
						} else {
                            t += v;
							console.log('cellType[k]: ' + cellType[k] + ' v: ' + v);
                        }
                        t += '</Data></Cell>';
                    }
                    k++;
